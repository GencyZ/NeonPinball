...
**Phase 1 三个决策（已定 ✅ 用户 2026-06-17 采纳倾向）**
| 决策 | 已定 | 一句话 |
|---|---|---|
| ① TOPUP_COUNT 默认 | ✅ **3** | 先用 3 跑，实机调；本就是头号旋钮 |
| ② Phase 1 就动 combo 曲线？| ✅ **不动** | 现在纯靠猜，先按原样实现、实机看了再调，免得调过头 |
| ③ 过渡保留阻塞？| ✅ **保留** | 改动最小最稳；实机觉得拖沓再去掉、做成打完即可瞄下一发 |

> 状态：✅ **已实现并推送**（2026-06-17，commits 7c30c57/c3cf950/df6b037/0cf8158，262 测试全绿）。三决策按表落地。**待实机调** TOPUP_COUNT/起始数/连击量级。
